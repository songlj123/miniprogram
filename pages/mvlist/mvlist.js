...
Page({
  ...
  async getMVlist(){
      ...
      let data = await request.req("/mv/first","get",{limit:10})
      //console.log(data)
      this.setData({songlistdata:data.data.data})
  ...
  async handleSongPlay(e){
    ...
    let songlistdata = await request.req("/mv/url","get",{id:e.target.id})
    //console.log(songlistdata)  
    this.setData({
      cursrc:songlistdata.data.data.url,
      curvideoid:e.target.id
    })
